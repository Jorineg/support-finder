    ...
    <div class="min-h-screen bg-surface flex flex-col">
      ...
      <div class="flex-1 flex items-center justify-center px-4 py-8">
        <div class="w-full max-w-2xl ${animClass}" id="question-card">
          <div class="bg-white rounded-2xl shadow-lg shadow-black/5 border border-gray-100 p-8 md:p-12">
...
function renderProgressBar(current, total) {
  ...
  return `
    <div class="w-full bg-gray-100 h-2 sticky top-0 z-10">
      <div class="bg-primary h-2 transition-all duration-500 ease-out rounded-r-full" style="width: ${pct}%"></div>
    </div>
    <div class="text-center pt-6 pb-2">
      <span class="text-sm font-medium text-gray-400 tracking-wide uppercase">${current + 1} of ${total}</span>
    </div>
  `;
}

function renderQuestionContent(q) {
  const infoBtn = `<button id="info-toggle" class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gray-100 hover:bg-primary-light text-gray-400 hover:text-primary transition-colors ml-2 align-middle flex-shrink-0" aria-label="More info">
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z"/></svg>
  </button>`;

  const infoPanel = state.infoOpen
    ? `<div class="mt-6 p-5 bg-info-bg border border-info-border rounded-xl text-base text-gray-600 leading-relaxed info-panel-enter">${q.detail}</div>`
    : "";
  ...
  return `
    <div class="flex items-start gap-1 mb-8">
      <h2 class="text-2xl md:text-3xl font-bold text-gray-900 leading-snug">${q.question}</h2>
      ${infoBtn}
    </div>
    ${infoPanel}
  ...
  return `
    <div class="grid grid-cols-2 gap-4">
      <button data-answer="true" class="yesno-btn group relative py-5 px-6 rounded-xl text-xl font-semibold border-2 transition-all duration-200 cursor-pointer
        ${yesActive ? "bg-primary text-white border-primary shadow-md shadow-primary/20" : "bg-white text-gray-700 border-gray-200 hover:border-primary hover:text-primary"}">
      ...
      <button data-answer="false" class="yesno-btn group relative py-5 px-6 rounded-xl text-xl font-semibold border-2 transition-all duration-200 cursor-pointer
        ${noActive ? "bg-primary text-white border-primary shadow-md shadow-primary/20" : "bg-white text-gray-700 border-gray-200 hover:border-primary hover:text-primary"}">
        <span class="flex items-center justify-center gap-3">
    ...
    <div class="flex flex-col items-center gap-3">
      <div class="relative w-full max-w-xs">
        <input id="number-input" type="number" value="${val}"
          ...
          class="w-full text-center text-3xl font-bold text-gray-900 bg-gray-50 border-2 border-gray-200 rounded-xl py-4 px-6 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
    ...
    <div class="flex flex-col gap-3">
      ${q.options
        ...
        <button data-value="${opt.value}" class="choice-btn w-full text-left py-4 px-6 rounded-xl text-lg font-medium border-2 transition-all duration-200 cursor-pointer
          ${currentAnswer === opt.value ? "bg-primary text-white border-primary shadow-md shadow-primary/20" : "bg-white text-gray-700 border-gray-200 hover:border-primary hover:text-primary"}">
          <span class="flex items-center gap-3">
            <span class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0
              ${currentAnswer === opt.value ? "border-white" : "border-gray-300"}">
...
function renderNextArea(question) {
  ...
  return `
    <button id="next-btn" ${!hasAnswer ? "disabled" : ""} class="flex items-center gap-2 py-3 px-8 rounded-xl text-lg font-semibold transition-all cursor-pointer
      ${hasAnswer ? "bg-primary text-white hover:bg-primary-hover shadow-md shadow-primary/20" : "bg-gray-100 text-gray-300 cursor-not-allowed"}">
...
function renderResults() {
  ...
  const cardHtml =
    eligible.length > 0
      ? eligible
          .map(
            (opt, i) => `
    <div class="fade-in bg-white rounded-2xl shadow-lg shadow-black/5 border border-gray-100 p-6 md:p-8" style="animation-delay: ${i * 80}ms; animation-fill-mode: both">
      <div class="flex items-start justify-between gap-4 mb-3">
        <div>
          <h3 class="text-xl font-bold text-gray-900">${opt.name}</h3>
          <p class="text-sm text-gray-400 mt-0.5">${opt.fullName}</p>
        </div>
        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap
          ${opt.type.includes("Scholarship") || opt.type.includes("Grant") ? "bg-success-light text-success" : opt.type.includes("Loan") ? "bg-amber-50 text-amber-700" : "bg-primary-light text-primary"}">
          ${opt.type}
        </span>
      </div>
      <p class="text-gray-600 leading-relaxed mb-4">${opt.description}</p>
      <div class="flex items-center justify-between pt-4 border-t border-gray-100">
        <span class="text-lg font-bold text-gray-900">${opt.maxAmount}</span>
        <a href="${opt.url}" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-primary hover:text-primary-hover font-medium transition-colors">
          Learn more
  ...
  `,
          ...
          .join("")
      : `
    <div class="fade-in bg-white rounded-2xl shadow-lg shadow-black/5 border border-gray-100 p-8 md:p-12 text-center">
      <div class="w-16 h-16 mx-auto mb-6 rounded-full bg-gray-100 flex items-center justify-center">
        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/></svg>
      </div>
  ...
  return `
    <div class="min-h-screen bg-surface">
      <div class="w-full bg-success h-2"></div>
      <div class="max-w-3xl mx-auto px-4 py-12">
        <div class="text-center mb-10 fade-in">
          <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-success-light flex items-center justify-center">
            <svg class="w-8 h-8 text-success" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Your Results</h1>
          <p class="text-lg text-gray-500">
            ${eligible.length > 0 ? `You may be eligible for <strong class="text-gray-900">${eligible.length}</strong> support option${eligible.length > 1 ? "s" : ""}` : "We've analyzed your answers"}
          </p>
        </div>
        <div class="flex flex-col gap-6">${cardHtml}</div>
        <div class="flex justify-center mt-12 gap-4">
          <button id="back-to-questions" class="flex items-center gap-2 py-3 px-6 rounded-xl text-lg font-medium text-gray-500 hover:text-gray-700 hover:bg-white border-2 border-gray-200 hover:border-gray-300 transition-all cursor-pointer">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
            Review Answers
          </button>
          <button id="restart-btn" class="flex items-center gap-2 py-3 px-6 rounded-xl text-lg font-semibold bg-primary text-white hover:bg-primary-hover shadow-md shadow-primary/20 transition-all cursor-pointer">
...
function renderWelcome() {
  return `
    <div class="min-h-screen bg-surface flex items-center justify-center px-4">
      <div class="max-w-2xl text-center fade-in">
        <div class="w-20 h-20 mx-auto mb-8 rounded-2xl bg-primary-light flex items-center justify-center">
        ...
        <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Student Support Finder</h1>
        <p class="text-xl text-gray-500 mb-2">Find financial support options for students in Germany</p>
        ...
        <button id="start-btn" class="inline-flex items-center gap-2 py-4 px-10 rounded-xl text-xl font-semibold bg-primary text-white hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all cursor-pointer">
          Get Started
    ...
    if (input) {
      input.addEventListener("input", () => {
        ...
        if (nextBtn) {
          ...
          nextBtn.className = `flex items-center gap-2 py-3 px-8 rounded-xl text-lg font-semibold transition-all cursor-pointer ${hasAnswer ? "bg-primary text-white hover:bg-primary-hover shadow-md shadow-primary/20" : "bg-gray-100 text-gray-300 cursor-not-allowed"}`;
        }
      });
